...
`SUPABASE_URL`, `SUPABASE_SERVICE_KEY`는 bot/manager가 공유하는 Supabase Postgres에 접속하기 위한 값입니다. `MANAGER_API_KEY`와 `INTERNAL_PORT`는 manager가 bot의 `/internal/notify`(기본 8765 포트)를 호출할 때 사용하며, `MANAGER_API_KEY`는 `X-API-Key` 헤더로 검증됩니다. 자세한 인바운드 규칙은 [oracle-cloud-vm-setup.md](E:\apps\supabot\docs\oracle-cloud-vm-setup.md) 를 참고하세요.
...
## 6-1. (Phase B 최초 1회) Supabase 데이터 마이그레이션

기존에 `data/users.json`, `data/orders.json`, JSONL 로그를 파일로 운영하던 경우, Supabase로 전환할 때 **최초 1회만** 마이그레이션을 실행합니다. 새로 구축하는 환경이라 이전할 파일 데이터가 없다면 이 단계는 건너뛰어도 됩니다.

먼저 `config/.env`에 `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`가 채워져 있어야 합니다(위 6단계 참고).
...
이 스크립트는 `data/users.json`, `data/orders.json`, 그리고 JSONL 로그를 Supabase 테이블로 옮깁니다. 최초 1회 실행이며, 옮긴 뒤에는 봇이 Supabase를 기준으로 동작합니다.
