...
## 틱사이즈 유틸리티
- `get_tick_size(price)` → Upbit/Bithumb 틱 단위 반환
- `adjust_price_to_tick(price)` → 위 틱으로 내림 (코인)
- `get_krx_tick_size(price)` / `adjust_krx_price_to_tick(price)` → KIS/Toss **국내** 주식 호가단위(원) 내림
- `adjust_us_price_to_tick(price)` → Toss **해외** 주식 센트(0.01) 단위 내림. `is_us_stock_ticker(exchange, ticker)`로 분기해서 호출해야 함 (`docs/detail/toss_overseas_stock.md`)
