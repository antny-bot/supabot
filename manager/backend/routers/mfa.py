...
async def api_login_mfa(request: Request):
    ...
    try:
        db = get_db()
        rows = (await db.table("users").select("mfa_secret,mfa_enabled").eq("user_id", user_id).execute()).data
...
async def api_mfa_setup(request: Request, user: dict = Depends(get_current_user)):
    email = user["email"]
    ...
    # 새로운 TOTP 보안키 생성
    mfa_secret = random_base32()
    provisioning_uri = TOTP(mfa_secret).provisioning_uri(
        name=email,
        issuer_name="supabot-manager"
    )

    # QR 코드를 백엔드에서 직접 생성하여 Base64 형태로 반환
    import qrcode
    import io
    import base64
...
async def api_mfa_enable(request: Request, user: dict = Depends(get_current_user)):
    user_id = user["bot_user_id"]
    if not user_id:
        return JSONResponse({"error": "연동된 봇 유저 정보가 없습니다."}, status_code=400)
    ...
    try:
        db = get_db()
        rows = (await db.table("users").select("mfa_secret").eq("user_id", user_id).execute()).data
    except Exception as e:
    ...
    # 검증 성공 시 최종 활성화
    try:
        await db.table("users").update({"mfa_enabled": True}).eq("user_id", user_id).execute()
    except Exception as e:
        return JSONResponse({"error": f"데이터베이스 업데이트 실패: {str(e)}"}, status_code=500)
...
async def api_mfa_disable(request: Request, user: dict = Depends(get_current_user)):
    user_id = user["bot_user_id"]
    if not user_id:
        return JSONResponse({"error": "연동된 봇 유저 정보가 없습니다."}, status_code=400)
