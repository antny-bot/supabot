...
| `users` | `GET /api/users`, approve/deactivate/activate/block/DELETE, `PATCH .../email`, `POST .../invite-auth-account` | admin only |
...
Supabase Auth invite API 호출 → 사용자에게 비밀번호 설정 메일 발송). SMTP/이메일 템플릿이
설정되어 있어야 메일이 실제로 발송된다.
