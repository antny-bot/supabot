...
- **Gemini 자연어 명령:** 텔레그램 일반 문장을 Gemini가 기존 명령 의도로 해석합니다. 조회는 즉시 실행하고 주문/설정 변경은 확인 버튼을 거칩니다.
- **견고한 예외 처리:** 
...
3. Gemini API 키를 입력하면 봇이 메시지를 삭제하고 `data/users.json`에 저장합니다.
...
Gemini API 키도 현재는 `data/users.json`에 평문으로 저장됩니다. 파일 권한은 가능한 `0600`으로 제한하지만, 다중 사용자 운영 또는 실서비스 전에는 암호화 저장이나 외부 시크릿 저장소 사용을 권장합니다.
