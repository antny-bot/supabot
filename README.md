...
- `/diag` : 관리자 전용 운영 진단, 보안 키/LLM/API 검증 캐시/폴링 상태 확인
...
5. 관리자는 `/nlstats`, `/nlstats export [N]`, `/nlstats hits`, `/nlstats clear confirm`으로 자주 들어오는 미처리 자연어 패턴과 전처리 hit, 추천 전처리 후보를 확인/정리할 수 있습니다.

## 🧪 운영 진단

관리자는 `/diag`로 봇 운영 상태를 한 번에 확인할 수 있습니다. `/diag`는 라이브 거래소 API를 호출하지 않고 저장된 설정과 최근 캐시만 표시합니다.

- 보안 키 상태: `USER_SECRET_KEY` 정상/없음/형식 오류/복호화 오류
- 거래 안전 상태: 수동 주문 확인 버튼 사용 여부, `max_order_krw`, KIS 실전/모의 환경
- API 검증 캐시: `/config`에서 키 저장 직후 수행한 마지막 검증 성공/실패 시각
- 최근 오류: `data/bot_events.jsonl`에 저장된 최근 warning/error 5건

`data/bot_events.jsonl`에는 API 키나 secret 값을 저장하지 않으며, 긴 토큰과 계좌성 숫자는 마스킹합니다.
