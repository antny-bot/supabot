...
- `/status` : **(NEW)** 가동 중인 트레이딩 전략 통합 대시보드 확인 (생략된 항목은 버튼으로 전체 펼치기)
...
### 2. 자산 및 시세 조회
- `/asset` : 통합 자산 현황 조회 (소액 자산 자동 요약, 버튼으로 거래소별 기타 종목 펼치기)
- `/price` (단축: `/p`) : 특정 종목의 실시간 시세 및 변동률 확인
- `/indicators` (단축: `/ind`) : RSI·MACD·볼린저밴드·스토캐스틱 멀티지표 한눈에 조회
- `/history` : 최근 체결 완료된 거래 내역 조회 (5건씩 버튼으로 페이지네이션)
- `/report` : 기간별 실현 손익 및 체결 요약 리포트 (종목 10개씩 버튼으로 페이지네이션)
...
- `/orders` : 현재 추적 중인 모든 미체결 주문 목록 (배치 그룹 요약, 버튼으로 전체 펼치기)
...
Oracle Cloud에서 SSH 복구와 컨테이너 기동까지 실제로 확인한 기록은 [Oracle Cloud SSH 복구 및 컨테이너 기동 성공 기록](docs/archived/oracle-cloud-ssh-recovery-success.md)을 참고하세요 (특정 시점 인스턴스 상태 기준 기록 — archive로 보관).
...
수파봇은 단순 주문 제출에 그치지 않고 가격 변동성과 기술적 지표에 실시간으로 대응하는 복합 트레이딩 전략 엔진을 탑재하고 있습니다. 전체 상세 사양은 [RSI 역산 및 전략 상세 명세](docs/301_rsi_algorithm.md)를 참고하세요.
